...
//輸入特價到期時間
var timestop = "2022/12/30 00:00:00";
var timestop1 = "2022/5/30 05:06:03";
var time = "time";
var time1 = "time1";
//  timestopFun(timestop, id);

timestopFun(timestop, time);
timestopFun(timestop1, time1);

function timestopFun(timestop, time01) {
    var time01 = time01;
    var setfu_time = timestop;
    var timeoutID = window.setInterval(function() {
        p(), newTime(setfu_time)
    }, 1000);
    ...
    //倒數計時
    function newTime(fu_time) {
        //未來時間
        var startTime = new Date(fu_time);
        //現在時間
        var endTime = new Date();

        //毫秒換成秒數
        var countDown = (startTime.getTime() - endTime.getTime()) / 1000;

        //換成天數 1天 = 24小时  1小时= 60分 1分 = 60秒
        ...
        //加到要顯示的盒子
        var html = "&nbsp;" + "<span>" + p(oDay) + "Day</span>" + "<span>" + "&nbsp;" + p(oHours) + ":</span>" + "<span>" + p(oMinutes) + ":</span>" + "<span>" + p(oSeconds) + "</span>";
        document.getElementById(time01).innerHTML = html;

        //時間到就要停止計數器
        if (countDown < 0) {
            document.getElementById(time01).innerHTML = '特價時間終止';
        }
    }
